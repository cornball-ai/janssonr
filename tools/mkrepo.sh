...
# arguments (rapt's pattern, served by GitHub Pages from main:/docs).
#
...
#   docs/pool/<suite>/main/r/janssonr/r-cornball-janssonr_<version>_<arch>.deb
#
# The repository is unsigned, so clients need 'Trusted: yes' in their
# .sources entry. To sign it instead, clearsign Release into InRelease and
# write a detached Release.gpg, then drop the plain copy below.
...
repo="$(cd "${JANSSONR_REPO:-$(dirname "$0")/../docs}" && pwd)"
pool="pool/${suite}/main/r/janssonr"
